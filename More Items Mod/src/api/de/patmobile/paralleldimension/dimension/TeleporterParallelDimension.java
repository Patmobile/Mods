...
public class TeleporterParallelDimension extends Teleporter {
	
    private static final BlockParallelDimensionPortal BLOCK_NETHER_PORTAL = (BlockParallelDimensionPortal) BlockInit.ParallelDimensionPortal;
    protected final WorldServer world;
    protected final Random random;
    protected final Long2ObjectMap<TeleporterParallelDimension.PortalPosition> destinationCoordinateCache = new Long2ObjectOpenHashMap<>(4096);

    public TeleporterParallelDimension(WorldServer worldIn) {
        super(worldIn);
        this.world = worldIn;
    ...
    public boolean placeInExistingPortal(Entity entityIn, float rotationYaw) {
        ...
        if (this.destinationCoordinateCache.containsKey(l)) {
            TeleporterParallelDimension.PortalPosition teleporter$portalposition = this.destinationCoordinateCache.get(l);
            d0 = 0.0D;
        ...
        if (d0 >= 0.0D) {
            if (flag) {
                this.destinationCoordinateCache.put(l, new TeleporterParallelDimension.PortalPosition(blockpos, this.world.getGameTime()));
            }
    ...
    public boolean makePortal(Entity entityIn) {
        ...
        IBlockState iblockstate = BLOCK_NETHER_PORTAL.getDefaultState().with(BlockParallelDimensionPortal.AXIS, l6 == 0 ? EnumFacing.Axis.Z : EnumFacing.Axis.X);
    ...
    public void removeStalePortalLocations(long worldTime) {
        if (worldTime % 100L == 0L) {
            ...
            ObjectIterator<TeleporterParallelDimension.PortalPosition> objectiterator = this.destinationCoordinateCache.values().iterator();

            while(objectiterator.hasNext()) {
                TeleporterParallelDimension.PortalPosition teleporter$portalposition = objectiterator.next();
